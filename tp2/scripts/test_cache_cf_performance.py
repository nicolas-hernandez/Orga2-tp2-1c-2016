    ...
    for v in versions:
        ...
        while tamX <= endSize:
            ...
            for i in xrange(Tcp.nInst):
                ...
                cmd = ['./tp2', '-v', "cropflip", '-i', v, Tcp.pathSW + Tcp.imgName + ".bmp"]
                
                '''
                cmdCG = ['valgrind', '--tool={0}'.format('cachegrind'), './tp2', '-v', "cropflip", '-i', v, Tcp.pathSW + Tcp.imgName + ".bmp"]
                '''
        ...
        with open(Tcp.tablesPath + "cropflip" + v + ".csv", 'wb') as csvfile:
